...
void drawCircleOutline(int xc, int yc, int r);
...
void player3(float x, float y, float scale)
{
    // Head
    glColor3f(0.57, 0.53, 0.3); // Skin color
    drawCircle(x, y + scale * 80, scale * 20, 50);

    // Body
    glColor3f(0.34, 0.82, 0.12);
    glBegin(GL_QUADS);
    glVertex2f(x - scale * 20, y + scale * 60);
    glVertex2f(x + scale * 20, y + scale * 60);
    glVertex2f(x + scale * 20, y);
    ...
    // Shoes
    glColor3f(0.84, 0.13, 0.37); // Gray color for shoes
    glBegin(GL_QUADS);
    // Left shoe
    glVertex2f(x - scale * 20, y - scale * 40);
    glVertex2f(x - scale * 5, y - scale * 40);
...
/// Cycle...........///
void drawCycle(int x, int y, float scale)
{
    ...
    // Draw First Circle Outline (Back Wheel)
    glColor3f(0.0, 0.0, 0.0);
    drawCircleOutline(-180, -40, 100);

    // Draw Inner Circle for Back Wheel
    drawFilledCircle(-180, -40, 20);

    // Draw Second Circle Outline (Front Wheel)
    glColor3f(0.0, 0.0, 0.0);
    drawCircleOutline(180, -40, 100);

    // Draw Inner Circle for Front Wheel
    drawFilledCircle(180, -40, 20);

    // Draw Middle Circle Outline
    glColor3f(0.0, 0.0, 0.0);
    drawFilledCircle(-10, -40, 30);
...
void drawCircleOutline(int xc, int yc, int r)
{
    ...
    for (int i = 0; i < num_segments; i++)
    {
        float x = r * cos(i * angle_step);
        float y = r * sin(i * angle_step);
...
void drawFilledCircle(int xc, int yc, int r)
{
    ...
    while (x <= y)
    {
        ...
        for (int i = -x; i <= x; i++)
        {
            glVertex2f(i + xc, y + yc);
            glVertex2f(i + xc, -y + yc);
        }
        for (int i = -y; i <= y; i++)
        {
            glVertex2f(i + xc, x + yc);
            glVertex2f(i + xc, -x + yc);
        }
        glEnd();
    }
...
// PlayGround//
    ...
    drawCycle(400, 700, 0.2f);  // Original size and position
